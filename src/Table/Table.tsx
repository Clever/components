...
export class Table extends React.Component {
  ...
  async _fetchLazy(pageIdx) {
    ...
    for (let idx = lazyPages.length; idx <= pageIdx; idx++) {
      const query = {pageSize, startingAfter: undefined};
  ...
  render() {
    ...
    return (
      <table className={classnames(CLASSNAMES.TABLE, fixed && CLASSNAMES.FIXED, className)}>
        ...
        <tbody className={CLASSNAMES.BODY}>
          {displayedData.length === 0 ? (
            <tr className={CLASSNAMES.ROW}>
              <Cell className={CLASSNAMES.NO_DATA} colSpan={columns.length} noWrap>
                {!pageLoading && "NO DATA"}
              </Cell>
            </tr>
            ...
            <tr
              className={classnames(
                CLASSNAMES.ROW,
                onRowClick && CLASSNAMES.CLICKABLE_ROW,
...
Table.propTypes = {
  ...
  rowIDFn:  React.PropTypes.func.isRequired,
  rowClassNameFn:  React.PropTypes.func,
