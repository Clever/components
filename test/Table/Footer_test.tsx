...
describe("Footer", () => {
  ...
  describe("scroll buttons", () => {
    ...
    it("disables 'Prev' button when on first page", () => {
      const scrollButtons = newFooter({ currentPage: 1, numPages: 2 }).find(
        `.${cssClass.BUTTON_SCROLL}`,
      );
    ...
    it("disables 'Next' button when on last page", () => {
      const scrollButtons = newFooter({ currentPage: 2, numPages: 2 }).find(
        `.${cssClass.BUTTON_SCROLL}`,
      );
    ...
    it("increments current page on 'Next' button click", () => {
      ...
      const scrollButtons = newFooter({ currentPage: 1, numPages: 2, onPageChange }).find(
        `.${cssClass.BUTTON_SCROLL}`,
      );
    ...
    it("decrements current page on 'Prev' button click", () => {
      ...
      const scrollButtons = newFooter({ currentPage: 2, numPages: 2, onPageChange }).find(
        `.${cssClass.BUTTON_SCROLL}`,
      );
  ...
  describe("page buttons", () => {
    ...
    it("collapses extra page numbers into ellipsis", () => {
      ...
      assert.equal(
        pageNumbers.length,
        expectedButtons.length + 2,
        "Incorrect number of page numbers displayed.",
      );
    ...
    it("highlights selected page number", () => {
      ...
      assert(
        selectedButton.equals(pageButtons.get(currentPage - 1)),
        "3rd page button should be selected.",
      );
    });
  });
});
