...
describe("ConfirmationButton", () => {
  ...
  it("calls onClose if present when cancel button clicked", () => {
    ...
    const confirmationButton = mount(<ConfirmationButton
      value="Confirm pl0x" modalTitle="A title" onClose={onCloseSpy}
    >
      <p>Hello</p>
    </ConfirmationButton>);
    confirmationButton.find("button").simulate("click"); // render the modal
    const cancelButton = confirmationButton.find(
      ".ConfirmationButton--dialog-buttons").find("button").first();
  ...
  it("calls onConfirm if present when confirm button clicked", () => {
    ...
    const confirmationButton = mount(<ConfirmationButton
      value="Confirm pl0x" modalTitle="A title" onConfirm={onConfirmSpy}
    >
      <p>Hello</p>
    </ConfirmationButton>);
    confirmationButton.find("button").simulate("click"); // render the modal
    const confirmButton = confirmationButton.find(
      ".ConfirmationButton--dialog-buttons").find("button").last();
