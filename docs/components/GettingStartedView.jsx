      ...
      <Grid>
        <Row grow className="margin--bottom--xl">
          <Col span={6} className="self--start padding--right--l">
            ...
            <p>Note: This library uses ES6 features not supported in Internet Explorer such as&nbsp;
            <a 
            ...
            <hr />
            <h2>Usage</h2>
            <p>In order to maintain visual consistency across our product, we use the components and styles from
               this library whenever possible.</p>
            <h3>Components</h3>
            <p>To use a component, you'll need to be working with React and Webpack. Components that include
            ...
            <hr />
            <h2>Styles</h2>
            <p>The components library exports a less file containing variables and classes for all of the styles
               outlined in the Design and LESS sections. To import these styles, just include the following line
               at the top of your less file:</p>
            <CodeSample>
              @import (reference) "~clever-components/dist/less/utilities";
            </CodeSample>
            <p>Note that by using <code className="text--small">(reference)</code>, you'll only import styles that
               you actually use, so it shouldn't bloat the resulting stylesheet.</p>
            <hr />
            <h2>Contributing</h2>
            <p>It's exciting to contribute to a project that helps scale work faster and easier. If you're
               interested in contributing to the design system, come by one of the frontend guild meetings or
               drop into the Slack channel.</p>
            ...
            <ul className="text--line-height-4">
              <li>Run <code className="text--small">./bin/new_component &#60;your component&#62;</code> to
                 autogenerate source and test stubs</li>
            ...
            <p>First, increment the version in <code className="text--small">package.json</code> according to
               semver. You can use the <code className="text--small">npm version</code> command (e.g.
               <code className="text--small">npm version major</code>).</p>
            <p>Then, when you merge your branch into <code className="text--small">master</code>, Drone will
               automatically build and publish a new version to npm.</p>
            ...
            <p>The components in the design system are built to be reusable, however, if the use case you need
               it for isn't supported, we suggest extending an existing component if possible instead of
               making an entirely new one.</p>
            <strong>A word of caution</strong>
            <p>Components are used throughout Clever's products. Modifying an existing component can impact
               multiple applications and we don't currently have a good way to detect UI regressions.
               Testing must be done manually.</p>
            <ul className="text--line-height-4">
